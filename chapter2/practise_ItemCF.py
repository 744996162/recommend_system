...
step3 : 将数据转化为 defaultdict(set)  getDict() key为物品ID
        {itemid1:set([userid1,userid1,..]), itemid2:set([userid1,userid2,..])}

step4 : 计算出邻接矩阵W
        W = defaultdict(dict)
        {itemid1:{itemid1:value1, itemid2:value2,...}, itemid1:{itemid1:value1, itemid2:value2},..}
...
def getDict_Item(list_data):
    dict_data_Item = defaultdict(set)
    for userid, itemid in list_data:
        dict_data_Item[itemid].add(userid)
    return dict_data_Item



def itemSimilarity(dict_data):
    W = defaultdict(dict)
...
def getRecommendItemIdList(w, itemid, k):
    """
    ...
    sorted_dict = sorted(item_dict.items(), key=lambda d: d[1], reverse=True)
    dict_recommendItemId = {}
    temp = 0
    for userid, value in sorted_dict:
        if temp >= k:
            break
        else:
            dict_recommendItemId[userid] = value
            temp += 1
    return dict_recommendItemId


def getRecommendItemId(dict_data, dict_recommendItemId, userid, topN):
    """
    dict_data 为输入数据 {itemid1:set([userid1,userid2,..]), itemid1:set([userid1,userid2,..])}
    ...
    for similar_userid, value in dict_recommendItemId.items():
